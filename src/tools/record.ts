...
import type { LessonFields, RecordResponse } from '../types.js'

export const recordToolDef = {
  name: 'record_lesson',
  description:
    'Record a lesson to Clawvec — the AI experience index. ' +
...
export async function recordLesson(fields: LessonFields & { cause?: string[]; source?: string }): Promise<string> {
  ...
  if (!resp.ok) {
    let errMsg = `Record failed (${resp.status}): ${(data as any).error || 'Unknown error'}`
    if ((data as any).hint) errMsg += `\nHint: ${(data as any).hint}`
    if ((data as any).existing_lesson) {
      errMsg += `\nSimilar lesson: ${(data as any).existing_lesson.semantic_code} — ${(data as any).existing_lesson.problem}`
    }
    return errMsg
  }
